...
function bind(kanna) {
    ...
    kanna.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    kanna.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    kanna.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    kanna.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   kanna.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    kanna.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    kanna.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    kanna.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    kanna.ev.on('chats.update', updates => {
    ...
    kanna.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    kanna.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**kanna.ev.on('messages.update', updates => {
    ...
    kanna.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    kanna.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    kanna.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (kanna === null || kanna === void 0 ? void 0 : kanna.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id) => {
            ...
            if (!message) {
                message = await (kanna === null || kanna === void 0 ? void 0 : kanna.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (kanna === null || kanna === void 0 ? void 0 : kanna.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return kanna === null || kanna === void 0 ? void 0 : kanna.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (kanna === null || kanna === void 0 ? void 0 : kanna.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (kanna === null || kanna === void 0 ? void 0 : kanna.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (kanna === null || kanna === void 0 ? void 0 : kanna.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (kanna === null || kanna === void 0 ? void 0 : kanna.messageInfo(remoteJid, id))
                if (msg) {
