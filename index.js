...
const connect = async () => {
    ...
    const kanna = new WAConnection(global.opts["legacy"] ? makeWALegacySocket(connOptions) : makeWASocket(connOptions))
    ...
    global.Store = Store.bind(kanna)

    kanna.ev.on("creds.update", saveState)

    kanna.ev.on("connection.update", async(update) => {
        if (update.connection == "open" && kanna.type == "legacy") {
            kanna.user = {
                id: kanna.state.legacy.user.id,
                jid: kanna.state.legacy.user.id,
                name: kanna.state.legacy.user.name
            }
        }
        ...
        if (connection == "close") {
            ...
            if (reason === DisconnectReason.badSession) { console.log(`Bad Session File, Please Delete Session and Scan Again`); kanna.logout(); }
            ...
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); kanna.logout(); }
            ...
            else kanna.end(`Unknown DisconnectReason: ${reason}|${connection}`)
        }
    })
    ...
    kanna.ev.on("messages.upsert", async (chatUpdate) => {
        m = serialize(kanna, chatUpdate.messages[0])

        if (!m.message) return
        if (m.key && m.key.remoteJid == "status@broadcast") return
        if (m.key.id.startsWith("BAE5") && m.key.id.length == 16) return
        require("./kanna")(kanna, m, Commands, chatUpdate)
    })

    if (kanna.user && kanna.user?.id) kanna.user.jid = jidNormalizedUser(kanna.user?.id)
    kanna.logger = (kanna.type == "legacy") ? DEFAULT_LEGACY_CONNECTION_CONFIG.logger.child({ }) : DEFAULT_CONNECTION_CONFIG.logger.child({ })
}
