...
module.exports = {
	...
	noLimit: true,
    start: async(kanna, m, {isPremium, isOwner}) => {
		let statuses
		try {
			statuses = await kanna.fetchStatus(m.sender);
		} catch {
			statuses = "Nothing.."
		}
		...
		try {
			var pp = await kanna.profilePictureUrl(m.sender, "image");
		} catch {
		...
		caption += `│\n`
		caption += `└───────⭓\n`
		kanna.sendFile(m.from, pp, "", m, { caption })
    }
}