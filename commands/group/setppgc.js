...
module.exports = {
    ...
    noLimit: true,
    start: async(kanna, m, { mime, quoted, text, prefix, command }) => {
        if (/image/.test(mime)) {
            let media = await kanna.downloadAndSaveMediaMessage(quoted)
            if (text.toLowerCase() === "original") {
                ...
                await kanna.updateProfilePicture(m.from, { url: media })
                .then( res => {
                    m.reply("Sukses")
                    fs.unlinkSync(media)
                }).catch(() => m.reply('err'))
