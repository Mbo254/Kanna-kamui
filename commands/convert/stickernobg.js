    ...
    start: async(kanna, m, { command, prefix, quoted, mime, text }) => {
        if (/image/.test(mime)) {
            ...
            request({
                ...
                await kanna.sendFile(m.from, ini_buff, "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] }).then(() => {
                    fs.unlinkSync(file_name)
                })
            })
        } else if (isUrl(text)) {
            kanna.sendFile(m.from, global.api("zenz", "/convert/sticker-nobg", { url: isUrl(text)[0] }, "apikey"), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
        }   else {
